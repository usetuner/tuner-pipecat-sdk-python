...
def _ttfb(processor, value_s):
    return _metric("TTFBMetricsData", processor=processor, value=value_s)
...
def test_llm_usage_accumulates():
    ...
    assert acc._pipecat_llm_total_tokens == 150
...
def test_tts_usage_accumulates():
    ...
    assert acc._pipecat_tts_chars == 275


def test_ttfb_llm_processor():
    acc = FlowsAccumulator()
    acc.on_metrics_frame(_frame(_ttfb("OpenAILLMService", 0.42)))
    assert acc._pending_pipecat_llm_ttfb_s == 0.42
    assert acc._pending_pipecat_tts_ttfb_s == 0.0


def test_ttfb_tts_processor():
    acc = FlowsAccumulator()
    acc.on_metrics_frame(_frame(_ttfb("ElevenLabsTTSService", 0.15)))
    assert acc._pending_pipecat_tts_ttfb_s == 0.15
...
def test_empty_frame_no_crash():
    ...
    assert acc._pipecat_llm_total_tokens == 0
    assert acc._pipecat_tts_chars == 0
...
def test_multiple_metric_types_in_one_frame():
    ...
    acc.on_metrics_frame(_frame(
        _llm_usage(300),
        _tts_usage(120),
        _ttfb("OpenAILLMService", 0.5),
        _processing("ElevenLabsTTSService", 0.9),
    ))
    assert acc._pipecat_llm_total_tokens == 300
    assert acc._pipecat_tts_chars == 120
    assert acc._pending_pipecat_llm_ttfb_s == 0.5
